...
# --- Types ---
ColorTransformFn = Callable[[np.ndarray], np.ndarray]
LuminanceFn = Callable[[np.ndarray], float]
SceneToDisplayLuminanceFn = Callable[[np.ndarray, LuminanceFn], np.ndarray]
...
def HLG_OOTF(e: np.ndarray, luminance: LuminanceFn) -> np.ndarray:
    y = luminance(e)
    return e * (y ** (OOTF_GAMMA - 1.0))


def HLG_OOTFApprox(e: np.ndarray,
                   luminance: LuminanceFn | None = None) -> np.ndarray:
    return np.power(e, OOTF_GAMMA)


def HLG_InvOOTF(e: np.ndarray, luminance: LuminanceFn) -> np.ndarray:
    y = luminance(e)
    return e * (y ** ((1.0 / OOTF_GAMMA) - 1.0))


def HLG_InvOOTFApprox(e: np.ndarray) -> np.ndarray:
    return np.power(e, (1.0 / OOTF_GAMMA))
...
BT709_TO_P3 = np.array([[0.822462,  0.177537,  0.000001],
                        [0.033194,  0.966807, -0.000001],
                        [0.017083,  0.072398,  0.91052]])

BT709_TO_BT2100 = np.array([[0.627404,  0.329282,  0.043314],
                            [0.069097,  0.919541,  0.011362],
                            [0.016392,  0.088013,  0.895595]])

P3_TO_BT709 = np.array([[1.22494,   -0.22494,   0.0],
                        [-0.042057,  1.042057,  0.0],
                        [-0.019638, -0.078636,  1.098274]])

P3_TO_BT2100 = np.array([[0.753833,  0.198597,  0.04757],
                         [0.045744,  0.941777,  0.012479],
                         [-0.00121,  0.017601,  0.983608]])

BT2100_TO_BT709 = np.array([[1.660491,  -0.587641, -0.07285],
                            [-0.124551,  1.1329,   -0.008349],
                            [-0.018151, -0.100579,  1.11873]])

BT2100_TO_P3 = np.array([[1.343578,  -0.282179, -0.061399],
                         [-0.065298,  1.075788,  -0.01049],
                         [0.002822, -0.019598,  1.016777]])


def Bt709ToP3(e: np.ndarray) -> np.ndarray:
    return ConvertGamut(e, BT709_TO_P3)


def Bt709ToBt2100(e: np.ndarray) -> np.ndarray:
    return ConvertGamut(e, BT709_TO_BT2100)


def P3ToBt709(e: np.ndarray) -> np.ndarray:
    return ConvertGamut(e, P3_TO_BT709)


def P3ToBt2100(e: np.ndarray) -> np.ndarray:
    return ConvertGamut(e, P3_TO_BT2100)


def Bt2100ToBt709(e: np.ndarray) -> np.ndarray:
    return ConvertGamut(e, BT2100_TO_BT709)


def Bt2100ToP3(e: np.ndarray) -> np.ndarray:
    return ConvertGamut(e, BT2100_TO_P3)


# YUV gamut conversion matrices
YUV_BT709_TO_BT601 = np.array([[1.0,      0.101579,   0.196076],
                               [0.0,      0.989854,  -0.110653],
                               [0.0,     -0.072453,   0.983398]])

YUV_BT709_TO_BT2100 = np.array([[1.0,     -0.016969,   0.096312],
                                [0.0,      0.995306,  -0.051192],
                                [0.0,      0.011507,   1.002637]])

YUV_BT601_TO_BT709 = np.array([[1.0,     -0.118188,  -0.212685],
                               [0.0,      1.018640,   0.114618],
                               [0.0,      0.075049,   1.025327]])

YUV_BT601_TO_BT2100 = np.array([[1.0,     -0.128245,  -0.115879],
                                [0.0,      1.010016,   0.061592],
                                [0.0,      0.086969,   1.029350]])

YUV_BT2100_TO_BT709 = np.array([[1.0,      0.018149,  -0.095132],
                                [0.0,      1.004123,   0.051267],
                                [0.0,     -0.011524,   0.996782]])

YUV_BT2100_TO_BT601 = np.array([[1.0,      0.117887,   0.105521],
                                [0.0,      0.995211,  -0.059549],
                                [0.0,     -0.084085,   0.976518]])


def YUV_Bt709ToBt601(e: np.ndarray) -> np.ndarray:
    return ConvertGamut(e, YUV_BT709_TO_BT601)


def YUV_Bt709ToBt2100(e: np.ndarray) -> np.ndarray:
    return ConvertGamut(e, YUV_BT709_TO_BT2100)


def YUV_Bt601ToBt709(e: np.ndarray) -> np.ndarray:
    return ConvertGamut(e, YUV_BT601_TO_BT709)


def YUV_Bt601ToBt2100(e: np.ndarray) -> np.ndarray:
    return ConvertGamut(e, YUV_BT601_TO_BT2100)


def YUV_Bt2100ToBt709(e: np.ndarray) -> np.ndarray:
    return ConvertGamut(e, YUV_BT2100_TO_BT709)


def YUV_Bt2100ToBt601(e: np.ndarray) -> np.ndarray:
...
def IdentityConversion(e: np.ndarray) -> np.ndarray:
    return e


def IdentityOOTF(e: np.ndarray, luminance: LuminanceFn) -> np.ndarray:
    return e
...
def ApplyToneMapping(rgb: np.ndarray, mode: ToneMapping,
                     target_nits: float = 100.0,
                     max_nits: float = 100.0) -> np.ndarray:
    return np.array(
